...
function spawnSafe(name,x,y){
    if(x>=0 && y>=0 && x<width && y<height) createPixel(name,x,y);
}
    ...
    for(let x=0;x<width;x++){
        for(let y=0;y<height;y++){
            ...
            if(p.element==="blood"){
                ...
                spawnSafe("neutrophil",x,y,0.004);
                spawnSafe("macrophage",x,y,0.003);
                spawnSafe("b_cell",x,y,0.002);
                spawnSafe("t_cell",x,y,0.002);
                spawnSafe("nk_cell",x,y,0.001);
            }
            ...
            // Pathogen mutation
            if((bacteria.concat(viruses).concat(fungi).concat(parasites).includes(p.element)) && chance(0.0005)){
...
console.log("🔥 Ultimate Body V5 Fully Working Sandboxels Mod Loaded 🔥");
